  ...
  switch ($_[0])
  {
    "L"	{ 
		   for ($i = 0 ; $i -lt $number ; $i++) { 
		     $xpos--
			 $steps++
			 if ( $Multi[$xpos,$ypos] -gt 0 ) {
				Write-host "X: (" $xpos "," $ypos ") Dist: " $([Math]::Abs($xpos-$ox) + [Math]::Abs($ypos-$oy)) " !! " $($Multi[$xpos,$ypos] + $steps) " ( " $Multi[$xpos,$ypos] " + " $steps " )"
                if ( ($Multi[$xpos,$ypos] + $steps) -lt $mindist) { $mindist = $Multi[$xpos,$ypos] + $steps } 		
			 }
		   }
		} 
	"R" {
