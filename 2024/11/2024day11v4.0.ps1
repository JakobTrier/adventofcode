...
# Version 4.0: A new approach; recursion 
...
$stones | ForEach-Object {
  $sum = CountStones $_ $blinks 
  write-host $(get-date -format "yyyy.MM.dd HH:mm:ss") "   Stone with number $($_) becomes $($sum) stones after $($blinks) blinks."
  $answer=$answer+$sum
}
